...
namespace Pinokio._3D.Eyeshot
{
    public partial class ViewPort
    {
        ...
        private bool ExtendEllipticalArc(ICurve ellipticalArcCurve, ICurve boundary, bool start)
        {
            ...
            Ellipse tempEllipse = new Ellipse(selEllipseArc.Plane, selEllipseArc.Center, selEllipseArc.RadiusX,
                selEllipseArc.RadiusY);
#if NURBS
                            ...
                            else
                            {
                                for (int i = 0; i < parameters.Count - 1; i++)
                                {
                                    if (u > parameters[i] && u <= parameters[i + 1])
                                    {
                                        curve.SplitBy(
                                            new Point3D[]
                                                {curve.PointAt(parameters[i]), curve.PointAt(parameters[i + 1])},
                                            out trimmedCurves);
                                    }
                                }
                            }
...
#if NURBS
                if (Curve.Chamfer((ICurve) firstSelectedEntity, (ICurve) secondSelectedEntity, distance, false, false,
                    true, true, out chamferLine))
                    AddAndRefresh(chamferLine, ActiveLayerName);
                else if (Curve.Chamfer((ICurve) firstSelectedEntity, (ICurve) secondSelectedEntity, distance, false,
                    true, true, true, out chamferLine))
                    AddAndRefresh(chamferLine, ActiveLayerName);
                else if (Curve.Chamfer((ICurve) firstSelectedEntity, (ICurve) secondSelectedEntity, distance, true,
                    false, true, true, out chamferLine))
                    AddAndRefresh(chamferLine, ActiveLayerName);
                else if (Curve.Chamfer((ICurve) firstSelectedEntity, (ICurve) secondSelectedEntity, distance, true,
                    true, true, true, out chamferLine))
                    AddAndRefresh(chamferLine, ActiveLayerName);
        ...
        private void CreateFilletEntity()
        {
            ...
            if (firstSelectedEntity is ICurve && secondSelectedEntity is ICurve)
            {
                ...
                try
                {
                    if (Curve.Fillet((ICurve) firstSelectedEntity, (ICurve) secondSelectedEntity, filletRadius, false,
                        false, true, true, out filletArc))
                        AddAndRefresh(filletArc, ActiveLayerName);
                    else if (Curve.Fillet((ICurve) firstSelectedEntity, (ICurve) secondSelectedEntity, filletRadius,
                        false, true, true, true, out filletArc))
                        AddAndRefresh(filletArc, ActiveLayerName);
                    else if (Curve.Fillet((ICurve) firstSelectedEntity, (ICurve) secondSelectedEntity, filletRadius,
                        true, false, true, true, out filletArc))
                        AddAndRefresh(filletArc, ActiveLayerName);
                    else if (Curve.Fillet((ICurve) firstSelectedEntity, (ICurve) secondSelectedEntity, filletRadius,
                        true, true, true, true, out filletArc))
                        AddAndRefresh(filletArc, ActiveLayerName);
                }
                catch
                {
        ...
        private void MoveEntity()
        {
            if (points.Count == 0)
            {
                DrawText(mouseLocation.X, Height - mouseLocation.Y + 10, "Select base point",
                    new Font("Tahoma", 8.25f), Color.Black, ContentAlignment.BottomLeft);
                return;
            }
            else if (points.Count == 1)
            {
                DrawText(mouseLocation.X, Height - mouseLocation.Y + 10, "Select second point",
                    new Font("Tahoma", 8.25f), Color.Black, ContentAlignment.BottomLeft);
        ...
        private void ScaleEntity()
        {
            ...
            if (points.Count == 0)
            {
                DrawText(mouseLocation.X, Height - mouseLocation.Y + 10, "Select origin",
                    new Font("Tahoma", 8.25f), Color.Black, ContentAlignment.BottomLeft);
            }
            else if (points.Count == 1)
            {
                DrawText(mouseLocation.X, Height - mouseLocation.Y + 10, "Select first reference point",
                    new Font("Tahoma", 8.25f), Color.Black, ContentAlignment.BottomLeft);
            }
            else if (points.Count == 2)
            {
                DrawText(mouseLocation.X, Height - mouseLocation.Y + 10, "Select second reference point",
                    new Font("Tahoma", 8.25f), Color.Black, ContentAlignment.BottomLeft);
        ...
        public void RotateEntity()
        {
            ...
            if (points.Count == 0)
            {
                DrawText(mouseLocation.X, Height - mouseLocation.Y + 10, "Select center of rotation",
                    new Font("Tahoma", 8.25f), Color.Black, ContentAlignment.BottomLeft);
            }
            else if (points.Count == 1)
            {
