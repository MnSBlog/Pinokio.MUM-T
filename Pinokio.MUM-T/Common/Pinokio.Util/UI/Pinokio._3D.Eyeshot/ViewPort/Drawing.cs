...
namespace Pinokio._3D.Eyeshot
{
    public partial class ViewPort
    {
        private void DrawInteractiveLines()
        {
            ...
            if (ActionMode == actionType.None && !ToolBar.Contains(mouseLocation) && points.Count > 0)
            {
                // Draw elastic line
        ...
        private Plane GetPlane4Rect(Point3D next)
        {
            Vector3D xAxis = new Vector3D(points[0], next);
            xAxis.Normalize();
            Vector3D yAxis = Vector3D.Cross(Vector3D.AxisZ, xAxis);
            ...
            if (ActionMode == actionType.None && !ToolBar.Contains(mouseLocation) && points.Count > 0)
            {
                ...
                if (points.Count == 2)
                {
                    ...
                    if (radius > 1e-3)
                    {
                        ...
                        if (System.Math.Abs(arcSpanAngle) > 1e-3)
                        {
        ...
        private Point3D GetSnappedPoint(System.Drawing.Point mousePos, Plane plane, IList<Point3D> pts, int indexToCompare)
        {
            ...
            if (pts.Count > 0)
            {
                ...
                Point2D current = new Point2D(mousePos.X, Height - mousePos.Y);
        ...
        public void DrawSelectionMark(System.Drawing.Point current)
        {
            ...
            double dim2 = Height - current.Y + (size / 2);
            double dim3 = current.X - (size / 2);
            double dim4 = Height - current.Y - (size / 2);
